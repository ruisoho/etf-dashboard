...
import type { ETFSectorAllocation } from '@/lib/api';

// Cache duration in seconds - sector allocation changes infrequently
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (provider) {
      ...
      result = await specificProvider.getSectorAllocation(symbol.toUpperCase());
    } else {
      // Use factory with fallback
      result = await factory.getSectorAllocation(symbol.toUpperCase());
    }
...
export type SectorAllocationAPIResponse = {
  success: boolean;
  data?: ETFSectorAllocation;
  error?: string;
  provider?: string;
  timestamp?: number;
  symbol?: string;
